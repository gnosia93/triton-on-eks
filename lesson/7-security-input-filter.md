...
Presidio에서 한국어 PII를 탐지하려면:

* Spacy의 한국어 모델(ko_core_news_lg 등)을 설치하거나
* 한국어 NER을 지원하는 커스텀 recognizer를 추가하거나
* Hugging Face의 한국어 NER 모델(예: monologg/koelectra-base-v3-nammed-entity-recognition)을 연동이 필요하다. 
이런 모델들은 수십~수백 MB 수준이고, CPU에서도 돌아가고, API 호출도 필요가 없다.
